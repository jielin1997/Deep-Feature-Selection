...
class MVTecAD(AnomalyDetectionDataset):
    ...
    available_categories = [  # categorise：类别
        "bottle",
        "cable",
        "capsule",
        "carpet",
    ...
    def __init__(
            self,
            # root: str = os.getenv("MVTECPATH", "/home/linjie/Work/images/PublicDataset/Mvtec_AD"),
            root: str = os.getenv("MVTECPATH", "/DATA_Inter/lj/backup/dataset/original/images/PublicDataset/Mvtec_AD"),
        ...
        if categories is None:
            ...
            self.categories = sorted(  # sorted()返回一个新的列表，按照升序包含可迭代对象中的所有项
                categories
            )  # Don't be needlessly undeterministic on swapped categories.

        # Load data from root
        self.classes = set()  # 创建一个无序不重复的元素集合
        self.samples = (
            []
        ...
        def add_class_images(class_folder: os.DirEntry) -> NoReturn:
            ...
            images = filter(
                lambda f: f.is_file() and (f.name.endswith(".png") or f.name.endswith(".jpg")),
        ...
        for category in self.categories:
            for dataset in train_test:
                ...
                # classes must be sorted to have completely deterministic dataset generation，类必须排序才能具有完全确定的数据集生成
                classes = sorted(
    ...
    def add_dataset_specific_args(
        ...
        parser.add_argument(
            "--category",
            type=str,
            choices=MVTecAD.available_categories,  # 参数choices表示参数可允许值的一个容器
            required=True,
